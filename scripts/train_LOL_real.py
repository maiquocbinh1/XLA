...
def main():
    ...
    if opts.resume_training_from_ckpt is not None:
...
def load_train_checkpoint(opts):
    ...
    opts['exp_dir'] = new_exp_dir
    
    opts['resume_training_from_ckpt'] = train_ckpt_path
    update_new_configs(opts, new_opts_dict)
    
    print(f"Resuming training. Output directory updated to: {opts['exp_dir']}")
    pprint.pprint(opts)
